...
- `assets/latido.svg` y `assets/titulo.svg` (animaciones del README) se regeneran con `python examples/latido.py --svg assets/latido.svg` y `python examples/titulo.py --svg assets/titulo.svg`. `assets/titulo.png` ya viene limpio (sin las líneas finas de la sombra de la tipografía original). El README usa URLs absolutas de GitHub porque también se muestra en PyPI.
